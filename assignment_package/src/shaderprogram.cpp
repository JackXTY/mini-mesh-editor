...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      attrPos(-1), attrNor(-1), attrCol(-1), attrJointIDs(-1), attrJointWeights(-1),
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifCamPos(-1), unifBind(-1), unifJointTran(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrJointIDs = context->glGetAttribLocation(prog, "vs_JIDs");
    attrJointWeights = context->glGetAttribLocation(prog, "vs_JWeights");

    unifModel      = context->glGetUniformLocation(prog, "u_Model");
    unifModelInvTr = context->glGetUniformLocation(prog, "u_ModelInvTr");
    unifViewProj   = context->glGetUniformLocation(prog, "u_ViewProj");
    unifCamPos     = context->glGetUniformLocation(prog, "u_CamPos");
    unifBind       = context->glGetUniformLocation(prog, "u_Binds");
    unifJointTran  = context->glGetUniformLocation(prog, "u_JointTrans");
}
...
void ShaderProgram::setJointInfo(const std::vector<Joint*>& joints)
{
    useMe();

    if(unifJointTran != -1)
    {
        glm::mat4 transMats[100] = {glm::mat4()};
        for(int i = 0; i < 100 && i < joints.size(); i++)
        {
            transMats[i] = joints[i]->getOverallTransformation();
        }
        context->glUniformMatrix4fv(unifBind, 100, false, &transMats[0][0][0]);
    }

    if(unifBind != -1)
    {
        glm::mat4 bindMats[100]  = {glm::mat4()};
        for(int i = 0; i < 100 && i < joints.size(); i++)
        {
            bindMats[i] = joints[i]->bind;
        }
        context->glUniformMatrix4fv(unifJointTran, 100, false, &bindMats[0][0][0]);
    }
}
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if(attrJointWeights != -1 && d.bindJointWeight())
    {
