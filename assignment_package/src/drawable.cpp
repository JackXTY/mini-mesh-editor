...
Drawable::Drawable(OpenGLContext* context)
    : count(-1), bufIdx(), bufPos(), bufNor(), bufCol(), bufJointIdx(), bufJointWeight(),
      idxBound(false), posBound(false), norBound(false), colBound(false),
      jointIDBound(false), jointWeightBound(false),
      mp_context(context)
{}
...
void Drawable::destroy()
{
    ...
    mp_context->glDeleteBuffers(1, &bufJointIdx);
...
{
    jointIDBound = true;
    // Create a VBO on our GPU and store its handle
    mp_context->glGenBuffers(1, &bufJointIdx);
}
